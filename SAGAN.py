...
class SAGAN(object):
    ...
    def __init__(self, sess, args):
        ...
        if self.dataset_name == 'mnist' :
            ...
            self.c_dim = 3
            self.data,self.lable = load_cifar10()
    ...
    def build_model(self):
        ...
        with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
            self.d_learning_rate = tf.train.cosine_decay_restarts(learning_rate=self.g_learning_rate, global_step=self.global_step,
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.epoch):
            ...
            for idx in range(start_batch_id, self.iteration):
                ...
                # update D network
                _, summary_str, d_loss, lr= self.sess.run([self.d_optim, self.d_sum, self.d_loss, self.d_learning_rate], feed_dict=train_feed_dict)
                ...
                if np.mod(idx+1, self.print_freq) == 0:
                    ...
                    #####################################
                    for i in range(manifold_h * manifold_w):
                        image_path = './' + self.sample_dir + '/' + self.model_name + '_train_{:02d}_{:05d}.png'.format(epoch, idx+1)
    ...
    def visualize_results(self, epoch):
        ...
        for i in range(image_frame_dim * image_frame_dim):
            image_path_ima = self.sample_dir + '/' + self.model_name + '_epoch%02d' % epoch + '_visualize.png'
            plt.imshow(samples[i], interpolation='nearest')
            plt.axis("off")
            plt.savefig(image_path_ima)
    ...
    def test(self):
        ...
        for i in range(self.test_num) :
            ...
            for j in range(image_frame_dim * image_frame_dim):
                image_path_test = result_dir + '/' + self.model_name + '_test_{}.png'.format(j)
                plt.imshow(samples[j], interpolation='nearest')
                plt.axis("off")
                plt.savefig(image_path_test)
